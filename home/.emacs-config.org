...
(define-key dired-mode-map "E" 'cpped-dired-ocr)
#+end_src
...
(defun cpped-show-contributor-info (files position type &optional start end)
  ...
  (let ((overlay (make-overlay position position))
        (text (concat (cpped-get-contributor-info files start end)
  ...
  (when (and buffer-file-name
    ...
    (cpped-show-contributor-info `(,buffer-file-name)
                                 (save-excursion
                                   (save-restriction
                                     (point-min)))
                                   'cpped-show-file-contributor-info)))
...
(defun cpped-org-insert-link-dwim (url)
  ...
  (org-insert-link nil (shark-bytes/shorten-url url) (cpped-url-to-title url)))
