...
*** Major mode icon
#+BEGIN_SRC emacs-lisp
(defun cpped-modeline-mode-icon (icon-face)
...
(with-eval-after-load "projectile"
  (defun cpped-modeline-buffer-id (icon-face)
...
(defun cpped-modeline-which-function ()
  (let ((current-function (substring-no-properties (or (when which-function-mode
                                                         (which-function))
                                                         ""))))
    (unless (= 0 (length current-function))
      (concat " " (truncate-string-to-width current-function 40 nil nil "…")))))
#+END_SRC
...
(defun cpped-modeline-check-status (face)
  "Collect check status information."
  (let ((status (mapconcat 'identity
                           (remove nil (mapcar 'funcall cpped-modeline-check-status-functions))
                           " ")))
              ...
              '((:eval
                 (let* ((mode-line-face (if (eq (frame-selected-window) cpped-selected-window)
                        ...
                        (left-side (mapconcat #'identity
                                              (-non-nil (list (cpped-modeline-mode-icon mode-line-face)
                                                              (cpped-modeline-project-id)
                                                              ...
                                                              (propertize (or (cpped-modeline-buffer-id mode-line-face)
                                                                              "")
                        ...
                        (right-side (mapconcat #'identity
                                          (-non-nil (list (cpped-modeline-process-running)
                                                          ...
                                                          (cpped-modeline-check-status (symbol-value 'mode-line-face))
                                                          ))
                                          "  ")))
