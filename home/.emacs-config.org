...
(use-package quelpa
  :config (setq quelpa-upgrade-p t
                quelpa-stable-p nil)
...
(use-package dashboard
  :after helm-buffers
  :config
  (setq dashboard-startup-banner (let ((image-url (with-current-buffer (url-retrieve-synchronously (format-time-string "http://dilbert.com/%Y-%m-%d"))
                                                    (goto-char (point-min))
...
(defun cpped-powerline-auto-format (icon-face)
  (when (and cpped-auto-format-enabled-p
    ...
    (propertize (all-the-icons-faicon "indent")
                'face (list ':family (all-the-icons-faicon-family)
                            ':background (face-attribute icon-face :background))
                'display '(raise -0.0))))
#+END_SRC
...
(defvar cpped-powerline-check-status-face-functions nil "A list of functions that compute the face for mode line status data.")

(defface cpped-check-status-info `((t :inherit default :background ,(face-attribute 'info :foreground)))
  "Face used for modeline check info status."
  :group 'cpped-powerline-check-status)

(defface cpped-check-status-warning `((t :inherit default :background ,(face-attribute 'warning :foreground)))
  "Face used for modeline check warning status."
  :group 'cpped-powerline-check-status)

(defface cpped-check-status-error `((t :inherit default :background ,(face-attribute 'error :foreground)))
...
(defun cpped-remaining-work-time (font-face)
  (when (and cpped-show-work-time
      ...
      (let* ((clocked-time (+ cpped-clocked-time-today (floor (- (float-time)
             ...
             (display-text (format "%d:%02d"
                                   (abs (/ difference 60))
                                   (abs (% difference 60)))))
        (if (< difference 0)
            (propertize (concat "-" display-text)
                        'face (list ':foreground (face-attribute 'error :foreground)
                                    ':background (face-attribute font-face :background)))
          (propertize (concat "+" display-text)
                      'face (list ':foreground (face-attribute 'info :foreground)
                                  ':background (face-attribute font-face :background)))))))
#+END_SRC
                ...
                '("%e"
                  (:eval
                   (let* ((active (powerline-selected-window-active))
                          ...
                          (check-status (mapconcat 'identity
                          ...
                          (check-face (when show-check-status
                                        (let ((faces (mapcar 'funcall cpped-powerline-check-status-face-functions)))
                                          (cond ((member 'cpped-check-status-error faces) 'cpped-check-status-error)
                                                ((member 'cpped-check-status-warning faces) 'cpped-check-status-warning)
                          ...
                          (lhs (list
                                ...
                                (when show-project
                                  (powerline-raw (cpped-powerline-docker-project (symbol-value 'face2)) face2 'l))
                                (powerline-raw (cpped-powerline-version-control) mode-line 'l)
                                (powerline-raw (cpped-powerline-eshell-process-running) mode-line 'l)
                          ...
                          (rhs (list
                                ...
                                (powerline-raw (cpped-remaining-work-time (symbol-value 'face1)) face1 'r))))
...
(use-package helm-ag
  :after (helm ag)
  :config
  (setq helm-ag-base-command "ag --smart-case"
...
(use-package desktop
  ...
  (run-with-idle-timer 300 t 'desktop-save)
...
(defun cpped-eshell-prompt-result ()
  (unless (= 0 0)
    (propertize (cpped-eshell-prompt-concat (propertize (all-the-icons-faicon "exclamation-triangle")
                                                        'face `(:family ,(all-the-icons-faicon-family))
                                                        'display '(raise -0.1))
...
(defun cpped-eshell-prompt-user ()
  (if (string= "root"
               (getenv "USER"))
      (concat (propertize (all-the-icons-faicon "user-plus")
                          'face `(:family ,(all-the-icons-faicon-family)
                                          :foreground ,(face-attribute 'warning :foreground))
                          'display '(raise -0.0)))
    (unless (string= (getenv "LOGNAME")
                     (getenv "USER"))
        (cpped-eshell-prompt-concat (propertize (all-the-icons-faicon "user")
                                                'face `(:family ,(all-the-icons-faicon-family))
                                                'display '(raise -0.0))
                                    (user-login-name)))))
...
(defun cpped-eshell-prompt-host ()
  (when (let ((host (getenv "SSH_CONNECTION")))
          (and host
               (not (string= "" host))))
    (cpped-eshell-prompt-concat (propertize (all-the-icons-faicon "desktop")
                                            'face `(:family ,(all-the-icons-faicon-family))
                                            'display '(raise -0.1))
                                (system-name))))
  ...
  (let* ((id (shell-command-to-string "cat /proc/self/cgroup | grep docker | head -n 1 | cut -d '/' -f3"))
         (container (when (and id
                      ...
                      (shell-command-to-string (concat "docker inspect -f '{{.Config.Image}}' "
                                                       id)))))
    (when container
      (cpped-eshell-prompt-concat (propertize "\xf395" ; docker
                                              'face `(:family ,(all-the-icons-faicon-family))
                                              'display '(raise -0.1))
                                  container))))

(defun cpped-eshell-prompt-path ()
  (cpped-eshell-prompt-concat (propertize (all-the-icons-faicon "folder-open")
                                          'face `(:family ,(all-the-icons-faicon-family))
                                          'display '(raise -0.1))
...
(use-package shell-pop
  ...
  :bind ("C-#" . shell-pop)
  :config
  (setq shell-pop-shell-type '("eshell" "*eshell*" (lambda nil
                                                     (cpped-eshell-maybe-new-session)))
...
(use-package flyspell
  ...
  (defun cpped-powerline-flyspell-status()
    ...
    (when (and flyspell-mode
      ...
      (concat (propertize (all-the-icons-faicon "comment")
                          'face (list ':family (all-the-icons-faicon-family))
                          'display '(raise -0.0))
  ...
  (add-to-list 'cpped-powerline-check-status-face-functions 'cpped-powerline-flyspell-status-face))
...
(defun cpped-docker-compose-file (project-root)
  "Find the compose file in the project root."
  (let ((compose-file (when default-docker-compose-file
                        (concat (file-name-as-directory project-root) default-docker-compose-file))))
    (when (and compose-file
             (file-exists-p compose-file))
        compose-file)))

(defun cpped-is-docker-project-p ()
  "Check if the current project is supports docker."
  (if (cpped-docker-compose-file (if (ignore-errors (projectile-project-p))
                                     (projectile-project-root)
                                   default-directory))
      t
    nil))

(defun cpped-maybe-run-in-docker (project-root command)
  "Run a command in docker if supported by the project."
  (let* ((compose-file (cpped-docker-compose-file project-root)))
    (if compose-file
...
(with-eval-after-load "flycheck"
  (setq flycheck-command-wrapper-function #'(lambda (command)
                                              (cpped-maybe-run-in-docker (if (ignore-errors (projectile-project-p))
                                                                             (projectile-project-root)
                                                                           default-directory)
                                                                         command))))
#+END_SRC
...
(use-package electric-operator
  ...
  :config (setq electric-operator-enable-in-docs t))
#+END_SRC
  ...
  (defun cpped-powerline-flycheck-status ()
    (when (and (boundp 'flycheck-last-status-change)
      ...
      (let* ((issues (let-alist (flycheck-count-errors flycheck-current-errors)
        ...
        (concat
         (propertize (all-the-icons-faicon "bug")
                     'face (list ':family (all-the-icons-faicon-family))
                     'display '(raise -0.0))
         (propertize (format " %d" issues)
                     'face (list ':weight 'bold))))))
  (add-to-list 'cpped-powerline-check-status-functions 'cpped-powerline-flycheck-status)
  (add-to-list 'cpped-powerline-check-status-face-functions 'cpped-powerline-flycheck-status-face))
#+END_SRC
...
(use-package highlight-function-calls
  :config
  (setq highlight-function-calls-not t)
                                    ...
                                    (lexical-let ((_files files))
                                      (lambda (candidate)
                                        (if (markerp candidate)
                                          ...
                                          (progn (set-buffer (org-capture-target-buffer (if (listp _files)
                                                                                            (car _files)
                                                                                          _files)))
                                                 (goto-char (point-max))
                                                 (org-capture-put-target-region-and-position)
...
(defhydra cpped-hydra-files (:exit t :hint nil)
  "
☰ File %s(concat (cpped-powerline-mode-icon) \" \" (cpped-powerline-buffer-id) \" \" (cpped-buffersize) \" \"  (cpped-file-encoding))
...
Open                             ^^Save                      ^^Close                  ^^Switch             ^^Version                    ^^Display                       ^^File                           ^^^^^^Encoding
      ...
      _n_: New                       _S_: Save as                _X_: Other files         _f_: Find            _d_: Diff                    _!_: Toggle read-only mode      _m_: Rename                      ^^^^_C-u_: UNIX
      _C_: From docker container                               ^^_w_: File and window                        ^^_g_: Log                     _h_: Hexadecimal mode           _D_: Diff to
                                   ^^_i_: Public gist            _u_: Unused files                           ^^_G_: Autocommit-Log                                        ^^_z_: Clear content
      _j_: Header/Source             _I_: Private gist           _a_: Close editor                           ^^_b_: Blame                   _6_: Decode Base64              _r_: Run command
      _t_: Test                                                                                          ^^^^^^_T_: Trace lines             _&_: Encode Base64              _c_: Copy name to clipboard
  _<RET>_: File under cursor         _e_: Encrypt                                                                                                                   ^^^^^^^^_._: Open directory
      _k_: Gist                                                                                                                                                   ^^^^^^^^^^_+_: Show Directory Tree
                                   ^^_F_: Toggle auto-format
"
  ("o" cpped-helm-find-files)
  ("n" helm-find-files)
  ("C" docker-container-find-file)
