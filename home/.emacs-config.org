...
(setenv "LC_PAPER" "de_DE.UTF-8")
(setenv "LC_NAME" "de_DE.UTF-8")
(setenv "LC_ADDRESS" "de_DE.UTF-8")
(setenv "LC_TELEPHONE" "de_DE.UTF-8")
(setenv "LC_MEASUREMENT" "de_DE.UTF-8")
(setenv "BROWSER" "eww")
(setenv "NINJA_STATUS" "[%p] ")
#+END_SRC
...
on load
 - set cmake-ide-build-dir if possible (locally)
(make-local-variable 'cmake-ide-build-dir)
(make-local-variable 'cpped-cmake-current-target)
...
(re-)config project: dir as config name

map (project dir -> (config, build dir)) saved to disk, unless dir removed
map (project dir -> current build dir) saved to disk
map (project dir -> target) saved to disk


(setq '((project-dir1 build1-dir1 build-dir2) (project-dir2 . build2-dir1)))

(push '() projects-alist)

current build-dir: (car (assoc (project-dir)))
...
(defun cpped-set-for-project-buffers (variable value)
(mapc (lambda (buffer)
        (with-current-buffer
(set variable value)))
(projectile-project-buffers))
...
(cpped-set-for-project-buffers 'cmake-ide-build-dir target)
(cpped-set-for-project-buffers 'cpped-cmake-current-target target)
...
(defun cpped-select-project-config
(interactive)
select form helm list + all
(cpped-switch-project-config config))




select config
set cmake-ide-build-dir for all buffers in project


(defun cpped-switch-project-config (config)
(interactive (list (if (cpped-project-build-directory-p config)
config
(read-directory-name "Build-directory: "
                                          (file-name-as-directory cmake-ide-build-pool-dir)))))
(if (cpped-project-build-directory-p config)
(cpped-set-for-project-buffers 'cmake-ide-build-dir config)
(error "`%s' is not a CMake build directory for this project" config)))
...
cov-coverage-file-paths function to find in build directory
add status to mode line (coverage <30%, >50% , >90%)
...
#+BEGIN_SRC emacs-lisp
(defun cpped-hex-to-rgba (string)
  "Convert a hexadecimal color definition to the rgba() notation."
  (interactive)
  (when (string-match (rx (or "#" "0x") (group (repeat 2 hex)) (group (repeat 2 hex)) (group (repeat 2 hex)) (optional (group (repeat 2 hex)))) string)
    (let ((alpha (match-string 4 string)))
      (concat "rgb"
              (when alpha
                "a")
              "("
              (format "%d" (string-to-number (or (match-string 1 string)
                                                 "0") 16))
              ", "
              (format "%d" (string-to-number (or (match-string 2 string)
                                                 "0") 16))
              ", "
              (format "%d" (string-to-number (or (match-string 3 string)
                                                 "0") 16))
              (when alpha
                (concat ", "
                        (format "%d" (string-to-number alpha 16))))
              ")"))))

(defun cpped-convert-color-component (component)
  "Map an integer or floating point color component to the 0-255 range."
  (min (max (if (floatp component)
                (truncate (* component 255.0))
              component) 0) 255))

(defun cpped-rgba-to-hex (string)
  "Convert a rgba() color definition to the hexadecimal notation."
  (interactive)
  (let ((alpha (match-string 4 string)))
    (when (string-match (rx "rgb" (optional "a") "(" (* " ")
                            (group (1+ (in numeric ".")))
                            (1+ (in " ,"))
                            (group (1+ (in numeric ".")))
                            (1+ (in " ,"))
                            (group (1+ (in numeric ".")))
                            (optional (1+ (in " ,")) (group (1+ (in numeric ".")))) ")")
                        string)
      (concat "#"
              (format "%02x" (cpped-convert-color-component (string-to-number (or (match-string 1 string)
                                                                                  "0"))))
              (format "%02x" (cpped-convert-color-component (string-to-number (or (match-string 2 string)
                                                                                  "0"))))
              (format "%02x" (cpped-convert-color-component (string-to-number (or (match-string 3 string)
                                                                                  "0"))))
              (when alpha
                (format "%02x" (cpped-convert-color-component (string-to-number (or (match-string 4 string)
                                                                                    "0")))))))))
...
***** [[https://github.com/redguardtoo/wucuo][Spell checking]]
#+BEGIN_SRC emacs-lisp
(use-package wucuo
  :config
  (setq wucuo-font-faces-to-check '(font-lock-string-face
                                    font-lock-doc-face
...
***** Auto-insert last variable
#+BEGIN_SRC emacs-lisp
;(defun c++-smart-dot ()
;  "Inserty a dot or an object name plus dot when appropriate."
; (interactive))
; (let (var-name)
  ;;   (if (and (looking-back "^[ \t]*")
...
(push '("T[Ee][Ss][Tt]_?.*\\.[Cc]([Cc]\\|[Pp]\\|[Xx]|++)\\'" . gtest-mode) auto-mode-alist)

Add test pattern to imenu
(add-to-list 'cc-imenu-c-generic-expression '("Test" "^[[:space:]]*TEST\\(_F\\)?(\\([^)]+\\))" 2))
(add-to-list 'cc-imenu-c++-generic-expression '("Test" "^[[:space:]]*TEST\\(_F\\)?(\\([^)]+\\))" 2))


(add-to-list 'cc-imenu-c++-generic-expression '("Test Class" "^[[:space:]]*#include[[:space:]]+[<\"]\\([^>\"]+\\)" 1))
class \\(\\):space:*: (public|protected) ::testing::Test


(add-to-list 'cc-imenu-c++-generic-expression '("SetUp" "SetUp:space:*(" 0))
(add-to-list 'cc-imenu-c++-generic-expression '("TearDown" "TearDown:space:*(" 0))
...
::testing::Test::RecordProperty("MaximumWidgets", ComputeMaxUsage());
...
Tip: If even this is too much work for you, you'll find the gmock_gen.py tool in Google Mock's scripts/generator/ directory (courtesy of the cppclean project)
useful. This command-line tool requires that you have Python 2.4 installed. You give it a C++ file and the name of an abstract class defined in it, and it will
print the definition of the mock class for you. Due to the complexity of the C++ language, this script may not always work, but it can be quite handy when it
...
(cpped-install-system-package "cog" :package-manager 'pip)

(defcustom cpped-cog-start-marker "[[[cog" "The marker string for the start of a cog block.")
(defcustom cpped-cog-end-marker "]]]" "The marker string for the end of a cog block.")
(defcustom cpped-cog-output-end-marker "[[[end]]]" "The marker string for the startend of a cog output block.")

(defcustom pm-inner/cog
  (pm-inner-chunkmode :name "cog-code"
                      :mode 'python-mode
                      :head-matcher (concat "^/[/\\*] *" (regexp-quote cpped-cog-start-marker))
                      :tail-matcher (concat "^/[/\\*] *" (regexp-quote cpped-cog-end-marker))
                      :head-mode 'host
                      :tail-mode 'host)
  "Inner mode for cog (python)."
  :group 'poly-innermodes
  :type 'object)

(define-polymode poly-c++-cog-mode
  :hostmode 'pm-host/C++
  :innermodes '(pm-inner/cog))

(defcustom pm-poly/c++-cog
  (pm-polymode-one "C++/cog"
                   :hostmode 'pm-host/C++
                   :innermodes '(pm-inner/cog))
   "C++/Cog polymode configuration."
   :group 'polymodes
   :type 'object)

(define-polymode poly-c++-cog-mode pm-poly/c++-cog)
(add-to-list 'auto-mode-alist '("\\.[Cc](C|c|PP|pp|XX|xx|++)\\'" . poly-c++-cog-mode))
(add-to-list 'auto-mode-alist '("\\.[Hh](H|h|PP|pp|XX|xx|++)\\'" . poly-c++-cog-mode))

(defvar cpped-cog-overlay-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-c C-c") 'cpped-cog-update-overlays)
    (define-key map (kbd "<f5>") 'cpped-cog-update-overlays)
    map)
  "Keymap for Cog overlays")

(defun cpped-cog-update-overlays()
  "Update all Cog overlays in the buffer."
  (interactive)
  (remove-overlays nil nil 'cpped-cog-result t)
  (when (not (save-restriction
               (save-excursion
                 (search-forward cpped-cog-start-marker nil t))))
    (let* ((original-buffer (current-buffer))
           (original-file (buffer-file-name original-buffer)))
      (with-temp-buffer
        (let ((cog-output (make-temp-file (file-name-nondirectory original-file) nil ".cog_processed")))
          (call-process "cog.py"
                        nil nil nil
                        (concat "--markers=" (s-join " "
                                                     `(,cpped-cog-start-marker
                                                       ,cpped-cog-end-marker
                                                       ,cpped-cog-output-end-marker)))
                        "-o" cog-output
                        original-file)
          (call-process diff-command
                        nil (current-buffer) nil
                        "--unified=0"
                        original-file
                        cog-output)
          (delete-file cog-output))
        (text-mode)
        (let ((pos (goto-char (point-min))))
          ...
          (while (not (eq (point) (point-max)))
            (pcase-let ((`(,buf ,line-offset ,pos ,old ,new ,switched)
              ...
              (let* ((overlay-begin (with-current-buffer buf
                                        (save-restriction
                                          (save-excursion
                                            (goto-char (car pos))
                                            (search-backward cpped-cog-start-marker nil t)))))
                     (overlay (make-overlay overlay-begin (car pos) buf)))
                (overlay-put overlay 'after-string (car new))
                (overlay-put overlay 'keymap cpped-cog-overlay-map)
                (overlay-put overlay 'cpped-cog-result t))
              (diff-hunk-next))))))))

(add-hook 'poly-c++-cog-mode-hook 'cpped-cog-update-overlays)
(add-hook 'after-save-hook 'cpped-cog-update-overlays)
#+END_SRC
...
***** Language Server
#+BEGIN_SRC emacs-lisp
(lsp-register-client (make-lsp-client :new-connection (lsp-stdio-connection '("cmake" "-E" "server" "--experimental"))
                                      :major-modes '(cmake-mode)
                                      :priority -1
                                      :server-id 'cmake-languageserver))
#+END_SRC
...
(with-eval-after-load "system-packages"
  ...
  (with-eval-after-load "flycheck"
    (flycheck-define-checker cpped-dockerlint
      ...
      :error-patterns ((error line-start "ERROR: " (message) (zero-or-one "on line " line) line-end)
                       (warning line-start "WARN: " (message) (zero-or-one "on line " line) line-end)
                       (info line-start "INFO: " (message) (zero-or-one "on line " line) line-end))
...
**** [[https://github.com/zaach/jsonlint][Syntax checker]]
#+BEGIN_SRC emacs-lisp
(with-eval-after-load "system-packages"
  (cpped-install-system-package "jsonlint" :package-manager 'npm)
  (with-eval-after-load "flycheck"
    (flycheck-define-checker cpped-jsonlint
      "JSON file checker (https://github.com/zaach/jsonlint)"
      :command ("jsonlint" "--quiet" source-original)
      :error-patterns ((error line-start "ERROR: " (message) (zero-or-one "on line " line) line-end)
                       (warning line-start "WARN: " (message) (zero-or-one "on line " line) line-end)
                       (info line-start "INFO: " (message) (zero-or-one "on line " line) line-end))
      :modes (json-mode))
    (add-to-list 'flycheck-checkers 'cpped-jsonlint)
    (add-hook 'json-mode-hook (lambda ()
                                (flycheck-select-checker 'cpped-jsonlint)))))
#+END_SRC
...
***** Autocompletion
#+BEGIN_SRC emacs-lisp
(defun cpped-company-xml-existing-tags-backend (command &optional arg &rest ignored)
  "Completes existing XML tags."
  (interactive (list 'interactive))
  (cl-case command
    (interactive (company-begin-backend 'cpped-company-xml-existing-tags-backend))
    ;(prefix (company-grab-symbol))
(prefix "<ca")
    (candidates
     (let ((matches))
...
(message "be start")
           (while (search-forward arg nil t)
             (backward-word)
(message (concat arg " -> " (word-at-point t)))
             (push (word-at-point t) matches))))
       matches))))

(add-to-list 'company-backends 'cpped-company-xml-existing-tags-backend)
...
#+BEGIN_SRC emacs-lisp
(cpped-install-system-package "trang")
;(add-to-list rng-schema-locating-files (expand-file-name "schema.xml" user-emacs-directory))
...
(defhydra cpped-hydra-ztree (:exit nil :hint nil :foreign-keys run)
  "
☰ Directory diff

Diff                    ^^Move                            ^^Edit              ^^View
  _d_: Change directory         _C-<tab>_: Switch side            _c_: Copy       _<tab>_: Toggle subtree
  _n_: Narrow               _<backspace>_: Move to parent     _<DEL>_: Delete
  _w_: Widen to parent                                                            ^^^^_f_: %s(if ztree-dir-show-filtered-files \"Hide\" \"Show\") filtered files
                                                                                ^^^^^^_=_: %s(if ztree-diff-show-equal-files \"Hide\" \"Show\") equal files
  _g_: Refresh
  _G_: Full Refresh

  _q_: Quit
"
  ("d" ztree-dir-change-directory)
  ("n" ztree-dir-narrow-to-dir)
  ("w" ztree-dir-widen-to-parent)

  ("g" ztree-diff-partial-rescan)
  ("G" (ztree-diff (car ztree-diff-dirs-pair)
                   (cdr ztree-diff-dirs-pair)))

  ("q" cpped-kill-default-buffer :exit t)

  ("C-<tab>" ztree-jump-side)
  ("<backspace>" ztree-move-up-in-tree)

  ("c" ztree-diff-copy)
  ("<DEL>" ztree-diff-delete-file)
  ("v" ztree-diff-view-file)
  ("e" ztree-diff-simple-diff-files)

  ("<tab>" ztree-toggle-expand-subtree)
  ("f" ztree-dir-toggle-show-filtered-files)
  ("=" ztree-diff-toggle-show-equal-files))
#+END_SRC
...
***** Show commits not merged to master
#+BEGIN_SRC emacs-lisp
;; (with-eval-after-load "magit"
;;   (progn
;;     (defun cpped-magit-insert-unmerged-commits()
;;       (magit-insert-section (unmerged-to-master)
;;         (magit-insert-heading "Unmerged to master:")
;;         (magit-log-wash-log "log" "--format=format:%h %s" "master..HEAD")))
;;     (magit-define-section-jumper cpped-magit-jump-to-unmerged-to-master "Unmerged to master" unmerged-to-master)
;;     (add-hook 'magit-status-sections-hook 'cpped-magit-insert-unmerged-commits t)
;;     (define-key magit-status-mode-map "jm" #'cpped-magit-jump-to-unmerged-to-master)))
#+END_SRC

***** Fix magit-diff-trace-definition
#+BEGIN_SRC emacs-lisp
(defun cpped-diff-trace-definition ()
  "Show git trace for current definition."
  (interactive)
  (let (buf pos)
    (save-window-excursion
      (magit-diff-visit-file (buffer-file-name (current-buffer)))
...
#+BEGIN_SRC emacs-lisp
;(quelpa '(git-undo :fetcher git :url "https://github.com/jwiegley/git-undo-el" :stable nil))
...
** Projects
#+BEGIN_SRC emacs-lisp
(defun cpped-closest-project-file (file)
  "Open the project's file closest to the current file."
  (interactive)
  (let ((directory default-directory))
    (when (ignore-errors (projectile-project-p))
      (while (not (string= directory (projectile-project-root)))
        (let ((filepath (concat (file-name-as-directory directory) file)))
          (when (file-exists-p filepath)
            filepath))))
    (concat (file-name-as-directory default-directory) file)))
#+END_SRC
...
CMake              ^^Git                           ^^Directories         ^^Shell                    ^^Other
  ...
  _x_: Close           _i_: Toggle ignore for file
                     ^^_I_: Open ignore file           _D_: Diff to ...

                     ^^_a_: Set file attributes
                     ^^_A_: Open attribute file
...
"
  ...
  ("l" magit-log-popup)
  ("i" (magit-gitignore (helm-projectile-find-file-dwim)))
  ("I" (find-file (cpped-closest-project-file ".gitignore")))
  ("a" (progn
         (find-file (cpped-closest-project-file ".gitattributes"))
         (ag-files "" "" )))
  ("A" (find-file (cpped-closest-project-file ".gitattributes")))
  ("v" magit-clone)
  ("V" magithub-clone)
  ("/" projectile-dired)
  ("." dired-jump)
  ...
  ("D" (ztree-diff (cpped-project-dir)))
...
Text                    ^^Code                                ^^Version Control         ^^System             ^^Other
  ...
  _f_: Define               _h_: Caller hierarchy                 _R_: Compare Regions      _I_: Info page
  _x_: Translate            _m_: Member hierarchy                 _T_: Trace definition
...
"
  ...
  ("+" er/expand-region)
  ("x" (if rectangle-mark-mode
           (call-interactively 'rectangle-exchange-point-and-mark)
         (exchange-point-and-mark)))
  ...
  ("e" mark-whole-buffer)
  ("c" (if rectangle-mark-mode
          ; (let ((kill-read-only-ok t)
          ;       (buffer-read-only t))
             (kill-rectangle (region-beginning) (region-end));)
         (call-interactively #'whole-line-or-region-kill-ring-save)) :exit t)
  ("d" (if rectangle-mark-mode
(progn
(message rectangle-mark-mode)
           (delete-rectangle (region-beginning) (region-end)))
         (call-interactively #'delete-region)) :exit t)
  ("k" (if rectangle-mark-mode
           (kill-rectangle (region-beginning) (region-end))
         (call-interactively #'whole-line-or-region-kill-region)) :exit t))
...
** Diffing
#+BEGIN_SRC emacs-lisp
(defhydra cpped-hydra-diff (:hint nil :exit nil :foreign-keys run)
  "
☰ Diff

Movement               ^^Edit                       ^^Apply
  _C-p_: Previous hunk     _C-x_: Split hunk            _C-a_: Apply hunk
  _C-n_: Next hunk         _C-r_: Reverse direction     _C-A_: Revert hunk
                         ^^_C-k_: Kill hunk
  _C-P_: Previous file     _C-K_: Kill file
  _C-N_: Next file
...
"
  ...
  ("C-x" diff-split-hunk)
  ("C-r" diff-reverse-direction)
  ("C-k" diff-hunk-kill)
  ("C-K" diff-file-kill)

  ("C-a" diff-apply-hunk)
  ("C-A" (diff-apply-hunk t))

  ("q" cpped-kill-default-buffer :exit t))

(add-hook 'diff-mode-hook 'cpped-hydra-diff/body)
#+END_SRC

** Merging
#+BEGIN_SRC emacs-lisp
(defhydra cpped-hydra-smerge (:hint nil :exit nil :foreign-keys run :pre (smerge-mode 1))
  "
☰ Merge

Movement           ^^Keep           ^^Resolve        ^^Diff
_<home>_: First        _b_: Base        _C_: Combine      _<_: Upper/Base
 _<end>_: Last         _u_: Upper       _r_: Resolve      _=_: Upper/Lower
     _k_: Previous     _l_: Lower       _k_: Remove       _>_: Base/Lower
     _j_: Next         _a_: All         _s_: Swap
                   ^^_RET_: Current
"
      ("<home>" (progn
                  (goto-char (point-min))
...
(add-hook 'smerge-mode-hook 'cpped-hydra-smerge/body)
#+END_SRC
...
cd: create dir hierarchy; complete up/downward dirs, locate, files (goto containing dir)
...
#+BEGIN_SRC emacs-lisp
(setq goto-address-url-regexp (replace-regexp-in-string "https://\\\\|" "https://\\|polarion: *\\|" goto-address-url-regexp nil 'literal))

(defun cpped-support-polarion-url (wrapped-function &rest arguments)
    "Detect Polarion URLs."
    (let ((start (line-beginning-position))
          (end nil))
      (save-excursion
        (backward-word 2)
        (setq start (max (point)
                         start)))
      (save-excursion
        (forward-word 2)
        (setq end (point)))
      (let ((words-at-point (buffer-substring-no-properties start end)))
        (message words-at-point)
        (if (string-match ".*[Pp]olarion: *\\([0-9]+\\)[^0-9]*" words-at-point)
            (concat "http://www.google.de/search?oq=" (match-string 1 words-at-point))
          (apply wrapped-function arguments)))))

(advice-add #'browse-url-url-at-point :around #'cpped-support-polarion-url)
#+END_SRC


C-x C-j will activate term-line-mode, which treats the terminal buffer more like a normal text-buffer in which you can move the cursor and yank text. You can switch back to character mode with C-c C-k.

** Linters
[[http://oclint.org/][OCLint]]
[[https://github.com/L2Program/FlintPlusPlus][Flint++]]
...
** Tools
[[https://github.com/markchalloner/git-semver][Git plugin for Semantic Versioning]]
[[http://semver.org/spec/v2.0.0.html][Semantic Versioning 2.0.0]]

** Info
[[https://github.com/jameslk/awesome-falsehoods][A curated list of awesome falsehoods programmers believe]]
[[https://github.com/kdeldycke/awesome-falsehood][Curated list of falsehoods programmers believe in.]]
[[https://github.com/onqtam/awesome-cmake][A curated list of awesome CMake resources, scripts, modules, examples and others.]]
[[https://github.com/unicodeveloper/awesome-tdd][Curated list of awesome resources: books, videos, articles about using TDD(Test Driven Development)]]
[[https://www.learnenough.com/git-tutorial][Learn Enough Git to Be Dangerous]]
...
[[https://fluentcolors.com/][Fluent Colors, Microsoft's Fluent Colors, Color Palette | FluentColors.com]]

** CMake
[[https://github.com/go2sh/cmake-integration-vscode][CMake Server Interface for VSCode]]
cmake target dependency view

** C++
auto-newline mode -> c-hanging-semi&comma-criteria
unfold include directive to includes used in unfolded header file
toggle {[(
toggle <" for headers
condition, namespace, function: jump to begin
automatic dot to pointer (->) if required
auto camelcase for known identifiers
show current symbol type and scope in mode line
show info above  method: references, tests passed (link to/run test), last change author, complexity
implement interface
macro (-> c-macro-expand)/template/constexpr/auto expansion
(project-wise) Find unused headers/sources
call graph
sequence diagram
show included header as inline text, allow edit
jump to base class/overridden method
paste to c++ string literal
include dependency visualizer
show deduced type for auto variable
mark const/non-const parameters/members
convert from explicit type to auto
delete matching bracket if bracket is deleted
Colorize Call Stacks
mark/dim logging lines
open non-project headers/sources as read-only
support compiler parseable fixits
embrace

[[https://sourceware.org/gdb/current/onlinedocs/gdb/Skipping-Over-Functions-and-Files.html][CC Mode: Custom auto newlines]]
[[http://accessmodifier.sourceforge.net/][AccessModifier Eclipse Plug-in]]
[[http://xray.inf.usi.ch/][XRay]]
[[https://www.codetrails.com/developer-assistance-tools/][Developer Assistance Tools | Codetrails]]
[[https://www.codetrails.com/error-analytics/][Error Analytics | Codetrails]]
[[http://marketplace.eclipse.org/content/data-hierarchy][Data Hierarchy]]
[[https://www.eclipse.org/tracecompass/][Trace Compass]]
[[https://www.wholetomato.com/features/default.asp][Features in Visual Assist]]
[[http://o.oz-code.com/features][OzCode: Features]]
[[https://www.cppdepend.com/][CppDepend :: C/C++ Static Analysis and Code Quality tool]]
[[https://www.sourcetrail.com/][Sourcetrail - A cross-platform source explorer for C/C++ and Java]]
** Refactoring
        surround with
        ...
        Toggle Function Definition moves a function definition inside an C/C++ source editor from one position to another and preserves correctness. (inline/header/source)
        ...
        create class from use
        add header for use
        create getters and setters
        override/implement function
        Extract (Function, Typedef, Variable, Parameters, Define, etc.) — here, CLion analyzes the block of code where you invoke the refactoring, detects the input and output variables and the usages of the selected expression, to replace them with a newly created entity of your choice.
        Replace includes with forward declaration(s)
        Organize includes
        Find unused includes
        implement method (header->inline, cpp->normal)
        remove unused headers
        renaming class renames files and includes
        rearrange methods (text list; can be reordered; grouped by access role)
        class generator: when inheriting pure virtual class, auto-add pure methods
        create case from switch type
        add default case
        invert if condition
        add/change to c++-cast
        change to auto
        rearrange methods by moving in header (changes source also)
        signature change: show editable list of changes in project


*** Yasnippet
implement switch for all enum values

*** Templight
[[https://github.com/mikael-s-persson/templight][Templight]]
[[https://github.com/mikael-s-persson/templight-tools][templight-tools]]
** Cog
[[https://nedbatchelder.com/code/cog/][Ned Batchelder: Cog]]

** Python
[[https://github.com/proofit404/anaconda-mode][proofit404/anaconda-mode]]

** QML
[[http://doc.qt.io/qt-5/qtqml-javascript-expressions.html][JavaScript Expressions in QML Documents]]
[[https://github.com/eclipse-cdt/cdt/commit/f9143ebfdc17541e9b44c65482952a1eb247382b][Bug 481126 - QML Static Analysis based on Tern.js · eclipse-cdt/cdt@f9143eb · GitHub]]
[[http://code.qt.io/cgit/qt-labs/vstools.git/tree/src/vsqml][Qt Visual Studio Add-in]]
[[https://github.com/Gaubee/acorn-qml][GitHub - Gaubee/acorn-qml: qml parser base on acorn]]
[[https://github.com/acornjs/acorn/find/master][File Finder · GitHub]]
[[https://github.com/mutaphysis/qml-tools][QML Coverage and other tools]]
[[https://www.froglogic.com/blog/measuring-qml-coverage/][Measuring QML Coverage]]
[[https://github.com/skozlovf/Sublime-QML][QML support for Sublime Text 2 and Sublime Text 3]]
qmltestrunner
qt quick test

** XML/XSLT
[[https://emacs.stackexchange.com/questions/20931/how-do-i-edit-an-xml-file-in-emacs-with-tag-autocompletion-from-a-known-xsd-sche][nxml - How do I edit an XML file in Emacs with tag autocompletion from a known XSD schema?]]
[[https://blog.xml.rocks/debugging-complex-xslt-modules/][Know-how: Debugging complex XSLT modules]]
[[https://sourceforge.net/projects/xslide/files/][xslide Emacs mode for XSL stylesheets]]
[[https://lgfang.github.io/mynotes/emacs/emacs-xml.html][Editing XML in Emacs]]
Schema compare
close xml tag on </
xsd-based aoutcomplete -> nxml-mode
surround selection with tag
XPATH test in XML doc

** Debugging
filter of methods for debugger not to step into
[[https://www.gnu.org/software/emacs/manual/html_mono/ccmode.html#Custom-Auto_002dnewlines][Debugging with GDB: Skipping Over Functions and Files]]
[[https://github.com/yyoncho/dap-mode][Debug Adapter Protocol for Emacs]]
start debugger and run to current line
debug: run to current line
boost visualizers
[[http://marketplace.eclipse.org/content/graphheap][GraphHeap]]
debug: restart on segfault; auto-step to last statement before faulty one
notes for break points
when debugging show which case of the if statement ran
array visualization (segments if too large)
debugger shows inline variable values

** Unit tests
[[https://github.com/piece/makegood/wiki][makegood]]
[[https://github.com/google/googletest/blob/master/googlemock/docs/ForDummies.md][googletest/ForDummies]]
[[https://github.com/google/googletest/blob/master/googlemock/docs/CookBook.md#mocking-nonvirtual-methods][googletest/CookBook]]
[[https://github.com/google/googletest/blob/master/googlemock/docs/CheatSheet.md][googletest/CheatSheet]]
[[http://www.ncrunch.net][NCrunch for Visual Studio]]
[[http://www.github.com/infinitest][Infinitest]]
test list

** Git
[[https://github.com/eamodio/vscode-gitlens/blob/master/README.md#gutter-blame-][vscode-gitlens]]
Locate Gitignore Files
magit-diff-trace-definition
Commits in Branch1 that are not in Branch2: git log Branch1 ^Branch2
...
** EDiff
[[https://www.gnu.org/software/emacs/manual/html_node/ediff/Major-Entry-Points.html#Major-Entry-Points][Major Entry Points - Ediff User's Manual]]
[[https://www.gnu.org/software/emacs/manual/html_node/emacs/Overview-of-Emerge.html][Overview of Emerge - GNU Emacs Manual]]
Conflict Solver: Take Both
*** Smerge-mode
Don't forget smerge-ediff
...
** Docker
[[https://docs.docker.com/engine/reference/commandline/logs/#options][docker logs | Docker Documentation]]

** Language Server
[[https://microsoft.github.io/language-server-protocol/specification][Specification]]

** org-mode
M-S-return (org-insert-todo-heading) inserts a new heading at the same level as the previous one
cpped-org-agenda-reschedule-to-today
org-occur
org-mode: org-babel-demarcate-block (split blocks, move point to next line first)

** Misc
[[https://github.com/google/bloaty][Bloaty McBloatface: a size profiler for binaries]]
[[https://github.com/namhyung/uftrace][namhyung/uftrace: Function (graph) tracer for user-space]]
[[https://github.com/Milys/NC-Gcode][Milys/NC-Gcode]]
Idea: keymap put into the text properties (or overlays)
project-wide replace with preview
json autocomplete: github.com/bali182/autocomplete-json
js: tern
suspicious code: github.com/MaxfieldWalker/neumann
electric-quote-chars (setq electric-quote-context-sensitive t)
The new function 'call-shell-region' executes a command in an inferior shell with the buffer region as input.
helm freedesktop application menu
...
*** editing
;; do not require mark to copy/kill
;; (require 'bind-key)
;; (bind-keys
