...
#+BEGIN_SRC emacs-lisp
(defvar cpped-dashboard-footer nil "Th dashboard footer text")

(use-package dashboard
  :after helm-buffers
  :hook (dashboard-mode . (lambda () (display-fill-column-indicator-mode -1)))
  :custom
...
(defun cpped-powerline-project-id ()
  ...
  (when buffer-location
      (let ((project-name (if (and (ignore-errors (projectile-project-p))
        ...
        (concat
         (cond ((cpped-tramp-file-remote-p buffer-location) "")
               ((not (= 0 (length project-name))) "")
                  ...
                  (:eval
                   (let* ((active (powerline-selected-window-active))
                          ...
                          (lhs (list
                                ...
                                (when show-project
                                  (powerline-raw (cpped-powerline-docker-project) mode-line 'l))
                                (when show-project
                                ...
                                (powerline-raw (cpped-powerline-remote) face2 'r)
                                (powerline-raw (cpped-powerline-su) face2 'r)
                                (powerline-raw (cpped-powerline-modified) face2)
...
(use-package devdocs-browser
  :quelpa ((devdocs-browser :fetcher github :repo "blahgeek/emacs-devdocs-browser"))
...
(defvar prettify-magit-alist (list (list (rx (= 7 (in "0-9a-f")) space (+ (in space punctuation "*" "/" "\\" "|")) (group "|")) "│")
                                   ...
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "sync" (? (or "s" "ed"))) eow) "痢")
                                   ...
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "optimize" (? (in "sd"))) eow) "輪")
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "set" (? "s")) eow) "=" '(:weight 'black :height 1.3))
                                   ...
                                   (list (rx bol (= 7 (in "0-9a-f")) space (+ (in space punctuation)) bow (group "lock" (? (or "s" "ed"))) eow) "𓄗")
...
(defun cpped-show-contributor-info (files position type &optional start end)
  ...
  (let* ((overlay (make-overlay position position)))
    (overlay-put overlay 'before-string (cpped-get-contributor-info files start end))
...
(use-package ob-docker-build
  ...
  :quelpa ((ob-docker-build :fetcher github :repo "ifitzpat/ob-docker-build"))
  :config
...
☰ Edit
  ...
  ___: Overwrite Mode                                                            ^^^^_C_: Color                                                                             ^^^^^^_o_: Split org-mode code block
...
Tools                      ^^Last Command    ^^System
  ...
  _b_: Bookmarks               _f_: Resume       _p_: Show processes
  _x_: Regex Builder                           ^^_s_: Show system services
  _e_: Evaluate expression                     ^^_P_: Manage system packages
                                             ^^^^_d_: Docker
