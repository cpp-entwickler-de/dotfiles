...
** [[https://github.com/Malabarba/elisp-bug-hunter][Config Debugger]]
#+begin_src emacs-lisp
(use-package bug-hunter
  :config
  (defun shark-bytes/debug-init-file ()
    (interactive)
    (bug-hunter-file (concat (file-name-sans-extension shark-bytes/config-file) ".el"))))
#+end_src
...
** Frame Setup
Show fullscreen for non-4k monitor
#+begin_src emacs-lisp
...
*** Determine base face at point (excluding highlighting)
#+begin_src emacs-lisp
(defun shark-bytes/face-at-point ()
  "Determine base face at point (excluding highlighting)."
  (let ((auto-highlight-symbol-mode-enabled auto-highlight-symbol-mode)
        (global-hl-line-mode-enabled global-hl-line-mode)
        (hl-line-mode-enabled hl-line-mode))
    ...
    (let ((face (get-char-property (point) 'face)))
      ...
      (unless face
        (error "No face found."))
      (auto-highlight-symbol-mode auto-highlight-symbol-mode-enabled)
      (hl-line-mode hl-line-mode-enabled)
      (global-hl-line-mode global-hl-line-mode-enabled)
      (if (listp face)
          (setq face (car face)))
      face)))
#+end_src

*** Show info about face at point
#+begin_src emacs-lisp
(defun shark-bytes/describe-face-at-point ()
  "Describe face at point."
  (interactive)
  (describe-face (shark-bytes/face-at-point)))
#+end_src

** [[https://github.com/rainstormstudio/nerd-icons.el][Icons]]
[[https://www.nerdfonts.com/cheat-sheet][Cheat sheet]]

#+begin_src emacs-lispset
...
(use-package dashboard
  ...
  (dashboard-footer-messages (let ((file (expand-file-name "~/.dashboard-messages")))
                               (if (file-exists-p file)
                                 ...
                                 '(""))))
  (dashboard-banner-logo-title (nth (random (length dashboard-footer-messages)) dashboard-footer-messages))
...
(with-eval-after-load "projectile"
  ...
  (defun shark-bytes/modeline-project-id ()
    (let* ((buffer-location (shark-bytes/buffer-name))
           (project-root (shark-bytes/project-dir))
           (project-name (projectile-project-name project-root))
           (remote (when (shark-bytes/tramp-file-remote-p buffer-location)
           ...
           (localname (if (tramp-tramp-file-p buffer-location)
                        ...
                        buffer-location)))
      (shark-bytes/truncate (concat (when remote
                                      (format " %s " remote))
...
(defun shark-bytes/modeline-caps-key-lock ()
  "Show state of caps lock key."
  (when shark-bytes/caps-lock-active
    ""))

(defun shark-bytes/modeline-num-key-lock ()
  "Show state of num lock key."
  (when shark-bytes/num-lock-active
    ""))
#+end_src
...
*** Display on top
#+begin_src emacs-lisp
(defface shark-bytes-mode-line-base '((t :inherit shark-bytes-ui)) "")
(defface shark-bytes-mode-line-emphasis '((t :inherit shark-bytes-mode-line-base :weight bold)) "")
(defface shark-bytes-mode-line-de-emphasis '((t :inherit shark-bytes-mode-line-base :height 0.7 :weight extra-light)) "")

(defvar shark-bytes/modeline-left-side-functions '(shark-bytes/modeline-mode-icon
                                                   shark-bytes/modeline-project-id
                                                   (shark-bytes/modeline-version-control . shark-bytes-mode-line-de-emphasis)
                                                   (shark-bytes/modeline-buffer-id . shark-bytes-mode-line-emphasis)
                                                   (shark-bytes/modeline-which-function . shark-bytes-mode-line-de-emphasis))
  "A list of functions to use for the right mode-line side.")

(defvar shark-bytes/modeline-right-side-functions '(shark-bytes/modeline-process-running
                                                    ...
                                                    shark-bytes/modeline-caps-key-lock
                                                    shark-bytes/modeline-num-key-lock
...
(defun shark-bytes/modeline-eval-functions (functions)
  "Evaluate mode-line functions safely."
  (mapconcat #'identity
             (-non-nil (mapcar #'(lambda (entry)
                                   (ignore-errors (let* ((value (funcall (or (car-safe entry)
                                                                             entry)))
                                                         (face (cdr-safe entry)))
                                                    (if (and value
                                                             face)
...
(setq-default header-line-format
              '((:eval
                 (let ((left-side (shark-bytes/modeline-eval-functions shark-bytes/modeline-left-side-functions))
                       (right-side (shark-bytes/modeline-eval-functions shark-bytes/modeline-right-side-functions)))
                    (face-remap-set-base 'header-line (if (mode-line-window-selected-p)
                                                          'mode-line
                                                        'mode-line-inactive))
                    (concat (propertize "  " 'display `(raise +0.4))
                                       left-side
                                       (propertize " " 'display `(space :align-to (- right ,(length right-side))))
...
** [[https://github.com/positron-solutions/transient-showcase][Transient]]
#+begin_src emacs-lisp
...
*** Hide completions buffer for ffap-menu
The command ffap-menu shows the *Completions* buffer by default, which is unnecessary with Vertico.
#+begin_src emacs-lisp
...
(use-package consult
  ...
  :bind (([remap apropos] . consult-apropos)
         ...
         (:map minibuffer-local-map ("M-h" . consult-history)))
  :custom
  ...
  (consult-point-placement 'match-beginning)
...
*** [[github:gonewest818/dimmer.el][Dim inactive buffers]]
#+begin_src emacs-lisp
(use-package dimmer
  :custom
  (dimmer-fraction 0.5)
...
(with-eval-after-load "magit"
  (defun eshell/gitroot ()
    "Change directory to current project root."
...
*** Bookmarks
**** [[github:akirak/bookmark-frecency.el][Sort by frecency]]
#+begin_src emacs-lisp
(use-package bookmark-frecency
  :custom (bookmark-sort-flag 'last-modified)
  :config (bookmark-frecency-mode))
#+end_src
...
(advice-add 'rename-visited-file :around 'shark-bytes/post-rename)
#+end_src
...
(defvar shark-bytes/consult-source-directory
  `(:name "Directory File"
    ...
    :items ,(lambda () (seq-remove '(lambda (directory)
                                       (string-match "\\.\\.?$" directory))
                                   (directory-files default-directory))))
  "Directory file candidate source for `consult'.")

(defvar shark-bytes/consult-source-recently-modified
  `(:name "Recently Modified File"
    :narrow ?m
    :category file
    :face consult-file
    :history file-name-history
    :state ,#'consult--file-state
    :enabled ,#'magit-inside-worktree-p
    :items ,(lambda ()
              (-non-nil (remove-duplicates (append (split-string (vc-git--run-command-string nil "ls-files" "-z" "-m" "-o" "--exclude-standard") "\0" t)
                                                   (split-string (shell-command-to-string "find -type f -mtime -7 ! -path './.git/*' -printf '%P\n'") "\n" t))))))
  "Recently modified file candidate source for `consult'.")

(defun shark-bytes/consult-find-file ()
  "Select file."
  (interactive)
  (when-let (buffer (consult--multi '(consult--source-recent-file
                                      shark-bytes/consult-source-recently-modified
...
**** [[github:rainstormstudio/nerd-icons-dired][Show Icons]]
#+begin_src emacs-lisp
...
***** [[https://github.com/rainstormstudio/treemacs-nerd-icons][Icons]]
#+begin_src emacs-lisp
...
*** [[github:DevelopmentCool2449/colorful-mode][Highlight colors]]
#+begin_src emacs-lisp
...
(use-package gumshoe
  ...
  :config
  (defun shark-bytes/gumshoe--consult-peruse (recs slot-spec &optional entry-filter)
    "Override for gumshoe--peruse to use 'consult--line'."
    (let ((candidates (mapcar (lambda (entry)
                                (let ((marker (make-marker)))
                                  (set-marker marker
                                              (slot-value entry 'position)
                                              (slot-value entry 'buffer))
                                  (when (and (marker-position marker)
                                             (marker-buffer marker))
                                    (consult--location-candidate (slot-value entry 'line)
                                                                 marker
                                                                 (with-current-buffer (slot-value entry 'buffer)
                                                                   (line-number-at-pos (slot-value entry 'position)))
                                                                 (slot-value entry 'position)))))
                              (if entry-filter
                                  (seq-filter (lambda (entry)
                                                (buffer-live-p (slot-value entry 'buffer))
                                                (funcall entry-filter entry))
                                              recs)
                                recs))))
      (consult--read candidates
                     ...
                     :state (consult--location-state (lambda () (funcall candidates nil)))
                     :group #'consult--line-multi-group)))
  (advice-add #'gumshoe--peruse :override #'shark-bytes/gumshoe--consult-peruse)
...
*** [[github:casouri/expreg][Expand Region]]
#+begin_src emacs-lisp
(use-package expreg)
#+end_src
...
***** [[github:fbrosda/yasnippet-treesitter-shim][Use yasnippet in treesitter-modes]]
#+begin_src emacs-lisp
(use-package yasnippet-treesitter-shim
  :after yasnippet
  :straight(yasnippet-treesitter-shim
            :host github
            :repo "fbrosda/yasnippet-treesitter-shim"
            :files ("snippets/*"))
  :no-require t
...
** [[https://github.com/minad/jinx][Spell checking]]
#+begin_src emacs-lisp
(use-package jinx
  ;:ensure-system-package (pkgconf enchant-devel enchant-aspell)
  :hook (emacs-startup . global-jinx-mode)
  :bind ([remap ispell-word] . jinx-correct-nearest)
  :custom
  (jinx-languages "en_US de_DE")
  :config
  (add-to-list 'jinx-camel-modes 'prog-mode)
  (defvar-local shark-bytes/spelling-errors 0 "The number of spelling errors in the document.")
  (advice-add 'jinx--check-pending :after (lambda (&rest arguments)
                                            (setq shark-bytes/spelling-errors (or (ignore-errors (length (jinx--force-overlays (window-start)
                                                                                                                               (window-end)
                                                                                                                               :visible nil
                                                                                                                               :check t)))
                                                                                  0))
                                            (force-mode-line-update)))
  (defun shark-bytes/modeline-spelling-status()
    "Format the modeline spelling information."
    (when (and jinx-mode
               (> shark-bytes/spelling-errors 0))
      (propertize (format " %d" shark-bytes/spelling-errors)
                          'face 'shark-bytes/check-status-warning)))
  (add-to-list 'shark-bytes/modeline-check-status-functions 'shark-bytes/modeline-spelling-status))
#+end_src
...
**** Highlighting
***** [[github:jdtsmith/indent-bars][Show Indentation]]
#+begin_src emacs-lisp
(use-package indent-bars
  ...
  (indent-bars-width-frac 0.11)
  (indent-bars-prefer-character nil)
  (indent-bars-pattern ".")
  (indent-bars-highlight-current-depth '(:width 0.3))
  (indent-bars-display-on-blank-lines t)
  (indent-bars-starting-column nil)
  (indent-bars-color '(default :face-bg t))
  (indent-bars-color-by-depth `(:palette (,shark-bytes-freezing
                                          ,shark-bytes-icy
                                          ,shark-bytes-cold
                                          ,shark-bytes-chilly
                                          ,shark-bytes-bleak
...
(use-package bug-reference
  ...
  :custom (bug-reference-auto-setup-functions 'bug-reference-try-setup-from-vc))
#+end_src

***** [[github:liuyinz/consult-todo][List TODOs]]
#+begin_src emacs-lisp
...
(use-package iedit
  :bind (:map iedit-mode-keymap ("C-g" . iedit-mode))
  :config
  (advice-add #'iedit--get-scope ; use function scope by default
              :filter-args
              (defun shark-bytes/iedit-defun-by-default (argument)
                (cond ((eq (car argument) nil) '(0))
                      ((eq (car argument) 0) '(nil))
                      (t argument)))))
#+end_src
...
****** [[github:LuigiPiucco/nerd-icons-corfu][Completion Icons]]
#+begin_src emacs-lisp
...
*** [[https://gitlab.com/wavexx/gcode-mode.el][G-Code-Mode]]
#+begin_src emacs-lisp
...
If the region is active, act on all lines touched by the region."
  ...
  (defun shark-bytes/magit-insert-user-info-header ()
    "Insert git user information."
    (insert (format "%-10s%s <%s>\n"
                    "User:"
                    (magit-git-string "config" "--get" "user.name")
                    (magit-git-string "config" "--get" "user.email"))))
  (add-hook 'magit-status-headers-hook #'shark-bytes/magit-insert-user-info-header))
...
**** [[github:KarimAziev/igist][Gist support]]
#+begin_src emacs-lisp
(use-package igist
...
(defun shark-bytes/enable-org-mode-prettification ()
  ...
  (push '(":tangle no" . ?⮿) prettify-symbols-alist)
  (push '(":dir " . ?🖿) prettify-symbols-alist)
  (push '(":output " . ?🖶) prettify-symbols-alist)
