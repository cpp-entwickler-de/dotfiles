...
(use-package projectile
  ...
  (defun shark-bytes/switch-project (&optional arg)
    "Load a project."
    (interactive "P")
    (if-let* ((projects (mapcar #'(lambda (project)
                                    (cons (file-name-base (directory-file-name project)) project))
                                (projectile-relevant-known-projects)))
              (selected-project (cdr (assoc (completing-read "Project: " projects) projects))))
        (projectile-switch-project-by-name selected-project arg)
      (user-error "There are no known projects")))
  (advice-add 'projectile-switch-project :override 'shark-bytes/switch-project)
  (with-eval-after-load "magit"
...
(defvar shark-bytes/default-customer-journal-items '(date begin end sum pause tasks) "A list of fields to build the journal entry string. Valid values are date, begin, end, sum, pause, tasks, nil for an empty field or a string, which will be formatted by 'format-time-string'. May be overwritten by 'shark-bytes/customer-journal-items'.")
...
(defvar shark-bytes/customer-journal-default-range 'daily "The default range for a journal entry. Valid values are daily or monthly. May be overwritten by 'shark-bytes/customer-journal-range'.")
(defvar shark-bytes/customer-journal-range nil "An alist of customer names and ranges for journal entries. Valid values are daily or monthly.")

(defun shark-bytes/clocked-customers (&optional range)
  "Return all customers with clocked hours."
...
(defun shark-bytes/collect-clocking-tasks (customer)
  ...
  (let ((range (org-clock-special-range (pcase (or (cdr (assoc customer shark-bytes/customer-journal-range)) shark-bytes/customer-journal-default-range)
                                          ('monthly 'thismonth)
                                          (_ 'today)))))
    (org-ql-select org-agenda-files
                   `(and (clocked :from ,(nth 0 range) :to ,(nth 1 range)) (category ,customer))
                   :action #'(lambda ()
                             (let ((element-end (org-element-property :end (org-element-at-point)))
                                   (range-start (time-to-seconds (nth 0 range)))
                                   (range-end (time-to-seconds (nth 1 range)))
                                   (task-data))
...
(defun shark-bytes/create-journal-entry (customer)
  ...
  (let* ((system-time-locale (or (cdr (assoc customer shark-bytes/customer-journal-date-format)) shark-bytes/customer-journal-default-date-format))
    ...
    (kill-new (string-join (mapcar #'(lambda (entry)
                                   ...
                                   (or (cdr (assoc customer shark-bytes/customer-journal-items))
                                       shark-bytes/default-customer-journal-items)) "\t"))
