...
(use-package consult
  ...
  (consult-customize consult-ripgrep consult-git-grep consult-grep consult-bookmark consult-recent-file consult-xref
...
(defvar shark-bytes/weekly-work-days 7 "The number of days of work per week.")
...
(defun shark-bytes/create-customer (customer)
  ...
  (let ((customer-project-directory (expand-file-name customer (car shark-bytes/project-directories))))
    ...
    (with-temp-buffer
      ...
      (insert (format "(push '(\"%s\" . '(:range '%s :items (%s) :date-format '%s)) shark-bytes/customer-journal-options)\n"
                      customer
                      (if-let ((journal-file (read-file-name "Journal file: " "~/Documents/" nil nil (concat " " customer))))
                          (format ":file \" \"" journal-file)
                        "")
                      (completing-read "Journal Range: " '(daily monthly))
...
(defvar shark-bytes/customer-journal-default-options '(:range 'monthly
                                                       :date-format 'de_DE
                                                       :duration-format h:mm
                                                       :items (date begin end sum pause tasks)
                                                       :unlisted-tasks '(Daily Misc))
...
(defun shark-bytes/customer-journal-option (customer option)
  ...
  (or (plist-get (cadr (assoc customer shark-bytes/customer-journal-options)) option)
...
(defun shark-bytes/collect-clocking-tasks (customer &optional range)
  ...
  (let ((log-range (org-clock-special-range (or range (pcase (shark-bytes/customer-journal-option customer :range)
                                                        ('weekly 'thisweek)
                                                        ('monthly 'thismonth)
                                                        (_ 'today))))))
    (-non-nil (org-ql-select org-agenda-files
                `(and (clocked :from ,(nth 0 log-range) :to ,(nth 1 log-range)) (category ,customer))
                :action #'(lambda ()
                            (let ((element-end (org-element-property :end (org-element-at-point)))
                                  (range-start (time-to-seconds (nth 0 log-range)))
                                  (range-end (time-to-seconds (nth 1 log-range)))
                                  (task-data))
