...
if [ "$PACKAGES" = TRUE ]; then
    # install packages
    sudo dnf install PackageKit-command-not-found adobe-source-code-pro-fonts adobe-source-sans-pro-fonts adobe-source-serif-pro-fonts atool autojump-zsh clang clang-analyzer clang11-devel clang-tools-extra cmake-gui colorgcc emacs-lucid fd-find fzf gcc gdb gdouros-symbola-fonts git git-delta git-filter-repo git-lfs htop ImageMagick inxi iotop kcachegrind links llvm-devel llvm11 llvm11-devel lnav ncdu progress pv python-devel recode ripgrep rubygems sushi util-linux-user valgrind wireless-tools xset yank zsh
