...
if [ "$PACKAGES" = TRUE ]; then
    # install packages
    sudo curl dnf install PackageKit-command-not-found adobe-source-code-pro-fonts adobe-source-sans-pro-fonts adobe-source-serif-pro-fonts autojump-zsh clang clang-analyzer clang-devel clang-tools-extra cmake-gui colorgcc emacs-lucid fd-find fzf gcc gdb gdouros-symbola-fonts git git-lfs htop ImageMagick iotop kcachegrind links llvm llvm-devel lnav ncdu progress pv python-devel recode rubygems sushi the_silver_searcher util-linux-user valgrind wireless-tools yank zsh
